...
//Rota de saúde da aplicaçãon (health check)
...
const USERS = [
    ...
    {id: 2, login: 'helton', password: '67891', name: 'Helton Azevedo'},
...
app.post('/login', (req, res) => {
    ...
    const user = USERS.find(u => u.login === plogin && u.password);
...
app.get("/conteudo-seguro", auth, async (req, res) => {
    res.status(200).json({error: false, message: `Usuário ${req.user}`});
});
